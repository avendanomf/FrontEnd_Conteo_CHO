...
export class CalculoChoComponent implements OnInit {
  ...
  @ViewChild(ResgitroComidasComponent) registroComidas: ResgitroComidasComponent | undefined;
  ...
  actualizarTotalCHO(filas: any) {
    this.registro.totalCHO = this.registro.totalCHO = filas.reduce((total: number, fila: any) => total + parseFloat(fila.gramosCarbohidratos), 0);
    this.registro.comidas = filas;
    this.calcInsulinaCHO();
    // console.log('desde el hijo: ' + JSON.stringify(registro.filas));
    // console.log('padre: ' + JSON.stringify(this.registro.filas));
  }
  calcInsulinaCHO() {
  ...
  limpiarCampos() {
    ...
    this.insulinas = new Insulinas();
    this.glucometrias = new Glucometrias();
    if (this.registroComidas) {
      this.registroComidas.restaurarCampos();
    }
  }
}
